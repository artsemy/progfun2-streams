...
class BloxorzSuite extends munit.FunSuite :
  ...
  test("neighborsWithHistory") {
    new Level1 :
      ...
      val exp: LazyList[(Block, List[Move])] = Set(
        (Block(Pos(1, 2), Pos(1, 3)), List(Right, Left, Up)),
        (Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up))
      ).to(LazyList)
      val act: LazyList[(Block, List[Move])] = neighborsWithHistory(Block(Pos(1, 1), Pos(1, 1)), List(Left, Up))
  ...
  test("newNeighborsOnly") {
    new Level1 :
      ...
      val act: LazyList[(Block, List[Move])] = newNeighborsOnly(
        Set(
          (Block(Pos(1, 2), Pos(1, 3)), List(Right, Left, Up)),
          (Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up))
        ).to(LazyList),
